# -*- coding: utf-8 -*-
"""
[올마나마] 단일 간장 키(cg_soy_sauce 등) -> 진간장/국간장/양조간장 정밀 분리 마이그레이션 스크립트 (v2)
...
# -------------------------------------------------------------------
# 간장 분류 키 및 매칭 규칙 (v2 정밀 키워드 예외 가드 포함)
# -------------------------------------------------------------------
...
EXCLUDE_GUK_WORDS = ["설탕", "설탕물", "사탕", "탕수육", "바탕"]

YANGJO_KEYWORDS = ["양념장", "드레싱", "소스", "무침", "겉절이", "비빔", "딥", "퐁당"]

def clean_text_for_guk(text):
    for ex in EXCLUDE_GUK_WORDS:
...
def classify_soy_sauce(dish_name, category_name, recipe_text):
    ...
    text_for_guk = clean_text_for_guk(text_combined)
    
    # 1. 국간장 키워드 검사 (설탕/탕수육 등 오탐 단어 제외 가드)
    for kw in GUK_KEYWORDS:
        if kw in text_for_guk:
