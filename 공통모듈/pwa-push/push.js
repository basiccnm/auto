// 웹푸시(Web Push) — VAPID 서명 + 무페이로드(tickle) 발송. 서비스워커가 문구를 채워 알림 표시.
// 페이로드 암호화(aes128gcm)는 범위 밖 — 무페이로드 푸시로 SW가 문구를 띄운다.
// Cloudflare Workers 네이티브(crypto.subtle), 외부 의존성 0.
// 출처: 에듀싱크(우리아이학교정보) site-renderer — 2026-07-16 실기기 검증본. 공통모듈 이식 2026-07-17.
//
// 필요한 env: VAPID_PRIVATE(32바이트 d, base64url), VAPID_PUBLIC(65바이트 비압축 공개키, base64url),
//             VAPID_SUBJECT("mailto:주소", 선택)
// 키 생성: node gen-vapid-keys.mjs
...
// 구독 하나에 무페이로드 푸시 발송. 반환: HTTP status(201=수락, 404/410=만료 → DB에서 삭제할 것).
