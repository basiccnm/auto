// 깻잎제육 반상편 판 — 여성향 톤 (2026-08-03)
// 🔴 ZicobaPage 를 그대로 복제했다. 골격·색을 건드리지 않는다 — 화면이 편마다 갈라지면 안 된다.
// 🔴 피자편(PizzaPage)과 골격은 같다. 색만 tone.js 를 따른다.
//    쇼츠는 JeyukPanel 만 떼어 쓰고, 스틸(JeyukPage)도 같은 걸 감싼다 → 화면이 갈라지지 않는다
...
export const JeyukPanel = ({ i = 0, top = 200, pop = 1 }) => (
  <Panel top={top} pop={pop}><Body p={PAGES[i] || PAGES[0]} /></Panel>
);

export const JeyukPage = ({ i = 0 }) => (
