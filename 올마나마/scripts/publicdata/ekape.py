...
def fetch_wholesale_recent(api, ymd, max_back=10):
    ...
    for back in range(max_back + 1):
        ...
        # ⚠️ 2026-07-19 실측: 경매가 없는 날에도 **행(부위·등급)은 그대로 내려오고
        #    금액 필드만 통째로 빠진다**(건수 0만 있음). 행 존재만 보고 반환하면
        #    "값이 전부 None인 하루"를 잡아버려 역탐색이 거기서 멈춘다.
        #    → 금액이 하나라도 있는 날만 유효한 경매일로 본다.
        #   건수(BoxCnt/Cnt)는 0이어도 int로 들어오므로 '값 존재'만 보면 안 된다.
        #   → **금액 필드(Avg/Amt/avg)가 0보다 큰 행**이 하나라도 있어야 유효한 경매일.
        def _has_price(row):
            return any(isinstance(v, int) and v > 0 and
                       ("avg" in k.lower() or "amt" in k.lower()) for k, v in row.items())
        if data and any(_has_price(row) for row in data.values()):
            return day, data
    return None, {}
