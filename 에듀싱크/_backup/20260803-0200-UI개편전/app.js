...
const STUB = {
  ...
  children: [
    { id: 1, nickname: "첫째", school: { slug: "서울숭미초등학교-7051136", name: "서울숭미초등학교", kind: "초등학교" },
    ...
    { id: 2, nickname: "둘째", school: { slug: "서울숭미초등학교-7051136", name: "서울숭미초등학교", kind: "초등학교" },
  ...
  school: { slug: "서울숭미초등학교-7051136", name: "서울숭미초등학교", kind: "초등학교",
    sido: "서울특별시", address: "서울 관악구 남부순환로 1926", phone: "02-871-0551",
    homepage: "http://sungmi.sen.es.kr", sem2_start: "20260820",
    // 학교알리미 공시값 — 2026-07-24 운영 D1에서 그대로 가져온 실제 값(전국 11,901개교 보유).
    detail: { class_count: 26, student_count: 487, teacher_count: 41, disclosure_round: "2026",
      grade_breakdown: [
        { grade: 1, students: 67, classes: 4, male: 34, female: 33 },
        { grade: 2, students: 60, classes: 3, male: 30, female: 30 },
        { grade: 3, students: 85, classes: 4, male: 49, female: 36 },
        { grade: 4, students: 75, classes: 4, male: 37, female: 38 },
        { grade: 5, students: 93, classes: 5, male: 48, female: 45 },
        { grade: 6, students: 94, classes: 4, male: 36, female: 58 },
      ] } },
  ...
  docSituations: [
    { key: "field",   icon: "<i class='ti ti-plane'></i>️", label: "가족여행 · 체험학습",
      desc: "교외체험학습 신청서\n실시 3일 전까지 · 승인되면 출석 인정" },
  ...
  // <i class='ti ti-alert-triangle'></i>️ 동명 학교가 많다(실측: 금성초 11곳·교동초 9곳·삼성초 9곳) → 목록에 반드시 주소를 같이 보여줘야 고를 수 있다.
  // 아래는 운영 D1에서 그대로 가져온 실제 행이다.
  schoolSearch: [
    { name: "금성초등학교", kind: "초등학교", sido: "서울특별시", address: "서울특별시 중랑구 신내로21길 55", slug: "금성초등학교-7021080" },
    { name: "금성초등학교", kind: "초등학교", sido: "부산광역시", address: "부산광역시 금정구 중리1길 15", slug: "금성초등학교-7191054" },
    { name: "금성초등학교", kind: "초등학교", sido: "대전광역시", address: "대전광역시 유성구 신성로72번길 5", slug: "금성초등학교-7451059" },
    { name: "금성초등학교", kind: "초등학교", sido: "경기도", address: "경기도 김포시 하성면 하성로 795번길 26", slug: "금성초등학교-7771020" },
    { name: "금성초등학교", kind: "초등학교", sido: "경상남도", address: "경상남도 진주시 말티고개로123번길 19", slug: "금성초등학교-9051012" },
    { name: "금성초등학교", kind: "초등학교", sido: "경상북도", address: "경상북도 의성군 금성면 탑리길 131", slug: "금성초등학교-8871028" },
    { name: "서울숭미초등학교", kind: "초등학교", sido: "서울특별시", address: "서울 관악구 남부순환로 1926", slug: "서울숭미초등학교-7051136" },
  ],
...
function homeCarousel() {
  ...
  if (meal && on("meal")) cards.push({ cls: "a", icon: "ti-bowl", top: "오늘 급식", sub: meal.dishes.slice(0, 2).map((x) => x.name).join(" · "), go: "S.schoolTab='meal';location.hash='#school';App.render()" });
  // 미제출 보고서
  ...
  if (on("dday")) {
    ...
    if (next) {
      ...
      cards.push({ cls: "a", icon: c2.icon, top: `${ddayLabel(next.date)} ${next.name}`,
        ...
        go: `S.schoolTab='schedule';S.calMonth='${next.date.slice(0, 6)}';location.hash='#school';App.render()` });
    }
  }
  // 최근 기록
...
const S = {
  ...
  missionCatalog: null,   // 고르기 시트에 띄울 목록 (null = 시트 닫힘)
  ...
  themeSheet: false,      // 내 정보의 테마 시트를 폈나
...
if (!DEV_TOOLS) {
  ...
  STUB.meals = []; STUB.schedules = []; STUB.timetable = { grade: "", class_name: "", neis_year: "", semester: "", grid: [[], [], [], [], [], []] };
}
...
const SUBJ_CI = { "국어": 0, "수학": 1, "영어": 2, "과학": 3, "사회": 4, "체육": 5,
  "음악": 6, "미술": 7, "창의적체험활동": 8, "실과": 3, "도덕": 4, "통합교과": 8, "안전한 생활": 5 };
const SUBJ_N = 9;
...
async function loadSchool(force) {
  ...
  try {
    const [info, meals, sched, tt] = await Promise.all([
      ...
      api(`${path}/timetable?grade=${encodeURIComponent(c.grade)}&class=${encodeURIComponent(c.class_name)}`),
    ]);
    SCHOOL.slug = slug;
    ...
    SCHOOL.from = "server";
    saveSchool(c);          // 다음에 켤 때 이걸로 먼저 그린다(같은 날·같은 반일 때만)
  } catch (e) {
...
const LEGAL = {
  ...
  terms: [
    ["제1조 (목적)", "본 약관은 베이직씨엔엠(이하 '회사')이 제공하는 '우리아이학교정보' 서비스(웹 및 모바일 앱, 이하 '서비스')의 이용 조건과 회사·이용자의 권리, 의무를 정합니다."],
...
function legalScreen(title, sections) {
  return `
  <a class="back" href="javascript:history.back()" style="font-size:22px;color:var(--muted)">‹</a>
  <h1 style="margin-top:8px">${esc(title)}</h1>
  <p class="sub">시행일 ${LEGAL.effective}</p>
  ${sections.map(([h, t]) => `
    <div style="margin-bottom:14px">
      <b style="display:block;font-size:14px;margin-bottom:4px">${esc(h)}</b>
      <p class="sub" style="margin:0;line-height:1.65">${esc(t).replace(/\n/g, "<br>")}</p>
    </div>`).join("")}`;
}
...
const Screens = {
  ...
  login: () => `
    <div class="lg">
      <div class="lg-hd">
        <div class="lg-mark" aria-hidden="true"><i></i><i></i><i></i><b></b></div>
        <div>
          <h1 class="lg-title">우리아이 학교정보</h1>
      ...
      <div class="lg-form">
        ...
        <div class="lg-field">
          ...
          <button class="lg-eye" onclick="App.pwShow()">${S.pwShow ? "숨기기" : "보기"}</button>
        </div>
      ...
      <div class="lg-links" style="margin-top:0"><a href="#help">고객센터 · 자주 묻는 질문</a></div>
    ...
    <div class="lg-child">
      <button onclick="location.hash='#childlink'">자녀폰 연결하기<em>6자리</em></button>
    </div>`,
  ...
  childlink: () => `
    <a class="back" href="#login" style="font-size:22px;color:var(--muted)">‹</a>
    <div class="kd2-link">
  ...
  findpw: () => {
    const f = S.findpw;
    const step = f.step;
    const err = (k) => (f.err[k] ? `<p class="fielderr">${esc(f.err[k])}</p>` : "");
    return `
    <a class="back" href="#login" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">비밀번호 찾기</h1>
    <p class="sub">가입할 때 적은 이메일로 <b>확인 코드</b>를 보내드려요.<br>
      로그인을 10번 틀려 잠겼을 때도 여기서 바로 풀 수 있어요.</p>

    <div class="steps">
      <span class="${step >= 1 ? "on" : ""}">1 본인 확인</span>
      <span class="${step >= 2 ? "on" : ""}">2 코드 입력</span>
      <span class="${step >= 3 ? "on" : ""}">3 새 비밀번호</span>
    </div>

    ${step === 1 ? `
      <div class="card">
        <label class="f">아이디 *</label>
        <input id="fp-userid" placeholder="가입한 아이디" value="${esc(f.userid)}" oninput="App.findpwSet('userid', this.value)">
        ${err("userid")}
        <label class="f">이메일 *</label>
        <input id="fp-email" placeholder="가입할 때 적은 이메일" value="${esc(f.email)}" oninput="App.findpwSet('email', this.value)">
        ${err("email")}
        <div style="margin-top:14px"><button class="btn-primary" onclick="App.findpwSend()">확인 코드 받기</button></div>
      </div>

      <!-- 2026-07-25 확정: 본인 확인은 **카카오 로그인 + 이메일** 두 갈래.
           카카오는 이미 붙어 있어 추가 비용이 0이고, 문자(건당 과금)는 안 쓴다.
           메일 발송은 도메인이 붙는 대로 연결한다(사장님: "도메인은 어차피 진행"). -->
      <div class="divider">또는 더 빠르게</div>
      <div class="social">
        <button onclick="App.findpwKakao()"><i class='ti ti-message-circle'></i> 카카오로 본인 확인하기</button>
      </div>
      <p class="notice"><i class='ti ti-info-circle'></i> 가입할 때 쓴 카카오 계정이면 코드 없이 바로 넘어가요.<br>
        아이디·이메일이 맞는지는 <b>서버가</b> 확인하고, 틀려도 "없는 계정"이라고 알려주지 않습니다 —
        남의 아이디가 있는지 떠보는 걸 막기 위해서예요.</p>`
    : step === 2 ? `
      <div class="card">
        <p class="sub" style="margin-top:0"><b>${esc(f.email)}</b> 으로 6자리 코드를 보냈어요.</p>
        <label class="f">확인 코드 *</label>
        <input inputmode="numeric" maxlength="6" id="fp-code" placeholder="6자리 숫자" value="${esc(f.code)}" oninput="App.findpwSet('code', this.value)">
        ${err("code")}
        <div style="margin-top:14px"><button class="btn-primary" onclick="App.findpwVerify()">확인</button></div>
        <div class="linkline" style="margin-top:10px;text-align:center;font-size:12px">
          <a href="#findpw" onclick="App.findpwSend()" style="color:var(--muted);padding:14px 6px">코드 다시 보내기</a>
        </div>
      </div>
      <p class="notice">코드는 <b>10분</b> 동안만 쓸 수 있어요. 메일이 안 오면 스팸함도 봐주세요.</p>`
    : step === 3 ? `
      <div class="card">
        <label class="f">새 비밀번호 *</label>
        <input type="password" id="fp-pw" placeholder="8자 이상, 영문+숫자" value="${esc(f.pw)}" oninput="App.findpwSet('pw', this.value)">
        ${err("pw")}
        <label class="f">새 비밀번호 확인 *</label>
        <input type="password" id="fp-pw2" placeholder="다시 한 번" value="${esc(f.pw2)}" oninput="App.findpwSet('pw2', this.value)">
        ${err("pw2")}
        <div style="margin-top:14px"><button class="btn-primary" onclick="App.findpwDone()">비밀번호 바꾸기</button></div>
      </div>
      <p class="notice">바꾸면 <b>로그인 잠금도 함께 풀리고</b>, 다른 기기에 남아 있던 로그인은 모두 끊어집니다.</p>`
    : `
      <div class="card" style="text-align:center;padding:26px 16px">
        <div style="font-size:40px;color:var(--good)"><i class='ti ti-circle-check'></i></div>
        <b style="display:block;margin-top:8px">비밀번호를 바꿨어요</b>
        <p class="sub">새 비밀번호로 로그인해 주세요.</p>
        <div style="margin-top:14px"><button class="btn-primary" onclick="location.hash='#login';App.render()">로그인하러 가기</button></div>
      </div>`}`;
  },

  // ── 2. 회원가입 — 계약 §3.3 필드 그대로. POST /api/v1/auth/register ──
  // 유효성은 입력하는 즉시 각 칸 아래에 표시한다. 실앱은 서버(중복 확인 등)도 같이 막아야 한다.
  register: () => {
    const r = S.reg;
  ...
  mission: () => {
    ...
    if (list === null) return `${childChip()}<h1>미션</h1><p class="sub">불러오는 중…</p>`;
    ...
    <div class="ms-sum">
      <b>${done}<span>/${list.length}</span></b>
      <span>${done === list.length ? "오늘 다 했어요" : `${list.length - done}개 남았어요`}</span>
      ${waiting.length ? `<em>확인 ${waiting.length}</em>` : ""}
    </div>

    ${list.map((x) => {
      const st = ST[x.status] || ST.open;
      return `
      <div class="ms-row ${st.cls}">
        <div class="ms-hd">
          <b>${esc(x.title)}</b>
          <span class="ms-tag">${st.ko}</span>
        </div>
        <div class="ms-meta">
          ${esc(MISSION_AREA[x.area] || "")} · ${x.minutes ? `${x.minutes}분 · ` : ""}★${x.stars}
          ${x.verify === "review" ? " · 사진 내기" : x.verify === "endday" ? " · 하루 끝에 확인" : ""}
          ${x.caution ? `<em class="ms-warn">${esc(x.caution)}</em>` : ""}
        </div>
        ${x.status === "waiting" ? `
          <div class="ms-photo">
            ${x.verify === "review" ? `
            <!-- 사진은 토큰이 붙어야 나온다 — data-rec 를 두면 hydrateImages() 가 받아 끼운다(기록 사진과 같은 방식) -->
            <img src="${PLACEHOLDER}" data-rec="/api/v1/missions/${x.id}/photo" alt="아이가 보낸 사진">`
            : `<p class="ms-said">아이가 <b>다 했어요</b>를 눌렀어요</p>`}
            <p class="sub">안 보시면 <b>내일 아침 8시</b>에 자동으로 도장이 나가요</p>
            <div class="row">
              <button class="btn-line" onclick="App.missionRevert(${x.id})">다시 해볼까?</button>
              <button class="btn-primary" onclick="App.missionApprove(${x.id})">확인했어요</button>
            </div>
          </div>` : ""}
        ${x.status === "done" || x.status === "skipped" ? `
          <div class="ms-act"><button class="mp-link" onclick="App.missionRevert(${x.id})">${x.status === "skipped" ? "다시 줘볼까?" : "다시 해볼까?"}</button></div>` : ""}
      </div>`;
    }).join("")}

    <div class="row" style="margin-top:16px">
      <button class="btn-line" onclick="App.missionPickOpen()">다른 걸로 바꾸기</button>
      <button class="btn-primary" onclick="location.hash='#home'">이대로 두기</button>
    </div>
    ...
    <button class="ms-mkbtn" onclick="location.hash='#report'">
      ...
      <span><b>이번 달 리포트</b><em>좋아하는 반찬 · 같이 논 친구 · 재밌었던 과목</em></span>
    </button>
    ...
    <button class="ms-mkbtn" onclick="App.missionMakeOpen()">
      ...
      <span><b>우리 집 미션 만들기</b><em>구몬 3장 · 윙크 20분처럼 우리 집에만 있는 것</em></span>
    </button>
    <p class="mp-hint">부모가 안 정해도 매일 아침 세 개가 자동으로 차요. 바꾸고 싶을 때만 바꾸면 됩니다.</p>

    ${S.missionCatalog === null ? "" : `
    <div class="modal sheetwrap" onclick="App.missionPickClose()">
      <div class="sheet tall" onclick="event.stopPropagation()">
        <div class="sheet-grip"></div>
        <div class="sheet-hd"><b>오늘 줄 미션 고르기</b><span>${S.missionPick.length}/3 · 사진만 셋이면 부담스러워요</span></div>
        <div class="ms-pick">
          <!-- 「구몬 3장」·「윙크 20분」처럼 집마다 다른 건 공용 목록에 못 담는다. 한 번 만들면 계속 쓴다. -->
          ${S.missionNew === null ? `
            <button class="ms-item ms-add" onclick="App.missionNewOpen()">
              <span class="ms-i1">＋ 우리 집 미션 만들기</span>
              <span class="ms-i2">구몬 3장 · 윙크 20분처럼 우리 집에만 있는 것</span>
            </button>` : `
            <div class="ms-form">
              <input id="mn-t" maxlength="20" placeholder="예: 윙크 20분 하기" value="${esc(S.missionNew.title)}"
                oninput="S.missionNew.title=this.value">
              <div class="ms-frow">
                <select onchange="S.missionNew.area=this.value">
                  ${Object.entries(MISSION_AREA).map(([k, v]) =>
                    `<option value="${k}" ${S.missionNew.area === k ? "selected" : ""}>${v}</option>`).join("")}
                </select>
                <select onchange="S.missionNew.minutes=+this.value">
                  ${[1, 3, 5, 10, 15, 20, 30].map((n) => `<option value="${n}" ${S.missionNew.minutes === n ? "selected" : ""}>${n}분</option>`).join("")}
                </select>
              </div>
              <!-- 「누르면 끝」은 이제 거짓말이다 — 의뢰는 전부 부모 확인을 거친다(2026-08-02) -->
              <div class="ms-frow">
                <select onchange="S.missionNew.verify=this.value">
                  <option value="instant" ${S.missionNew.verify === "instant" ? "selected" : ""}>눌러서 보고</option>
                  <option value="review" ${S.missionNew.verify === "review" ? "selected" : ""}>사진 내기</option>
                  <option value="endday" ${S.missionNew.verify === "endday" ? "selected" : ""}>하루 끝에 확인</option>
                </select>
              </div>
              <p class="ms-fhint">★는 시간에 따라 붙어요 — 5분 미만 ★1 · 5~14분 ★2 · 15분 이상 ★3.
                확인은 부모가 하고, 안 보시면 다음날 아침 8시에 자동으로 나갑니다.</p>
              <div class="row">
                <button class="btn-line" onclick="App.missionNewClose()">취소</button>
                <button class="btn-primary" onclick="App.missionNewSave()">만들기</button>
              </div>
            </div>`}
          ${(S.missionCatalog || []).map((m) => {
            const on = S.missionPick.includes(m.code);
            return `
            <button class="ms-item ${on ? "on" : ""}" onclick="App.missionToggle('${jsq(m.code)}')">
              <span class="ms-i1">${m.mine ? `<i class="ms-mine">우리 집</i>` : ""}${esc(m.title)}</span>
              <span class="ms-i2">${esc(MISSION_AREA[m.area] || "")} · ${m.minutes ? `${m.minutes}분 · ` : ""}★${m.stars}
                ${m.verify === "review" ? " · 사진" : m.verify === "endday" ? " · 안 하기" : " · 바로"}
                ${m.caution ? `<em class="ms-warn">${esc(m.caution)}</em>` : ""}
                ${m.mine ? `<em class="ms-del" onclick="event.stopPropagation();App.missionCustomDelete('${jsq(m.code)}')">지우기</em>` : ""}</span>
            </button>`;
          }).join("")}
        </div>
        <div class="sheet-act nodel">
          <button class="act-cancel" onclick="App.missionPickClose()">취소</button>
          <button class="act-del" style="background:var(--accent);color:var(--on-accent)" onclick="App.missionSave()">이걸로 주기</button>
        </div>
      </div>
    </div>`}`;
  },
  ...
  help: () => {
    ...
    return `
    <a class="back" href="javascript:history.back()" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">고객센터</h1>
    <p class="sub">궁금한 것을 먼저 찾아보시고, 없으면 바로 물어보세요</p>
    ...
    ${q ? `<p class="sub" style="margin:10px 2px 0">${hitCount ? `${hitCount}개 찾았어요` : "찾는 답이 없네요. 아래에서 바로 물어보세요"}</p>` : ""}
    ...
    <div class="card" style="margin-top:18px">
      ...
      <p class="sub" style="margin:0 0 12px">보통 1~2 영업일 안에 답을 드려요. 앱 안에서 답변을 보실 수 있습니다.</p>
    ...
    <div class="card" style="margin-top:12px">
      ...
      <div class="row" style="margin-top:10px">
        <button class="btn-line" onclick="location.hash='#terms'">이용약관</button>
        <button class="btn-line" onclick="location.hash='#policy'">개인정보처리방침</button>
      </div>
    </div>`;
  },
  ...
  ask: () => {
    ...
    const c = cur();
    return `
    <a class="back" href="#help" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">문의하기</h1>
    ...
    <textarea id="ask-body" maxlength="2000" placeholder="언제, 어느 화면에서, 무엇이 어떻게 되었는지 적어주시면 훨씬 빨리 해결돼요."
      ...
      : list.map((x) => {
        ...
        <div class="card hlp-item ${open ? "on" : ""}">
          <button onclick="App.askOpenOne(${x.id})">
            <span class="hlp-hd">
              <b>${esc(x.title)}</b>
              <span class="${cls}">${ko}${x.status === "answered" && !x.seen_at ? " ·" : ""}</span>
            </span>
          ...
          ${open ? `
            <div class="hlp-body">
              ...
              ${x.answer ? `<p class="hlp-ans"><b>답변</b>${esc(x.answer)}</p>`
                : `<p class="sub" style="margin:0">아직 답변 전이에요. 보통 1~2 영업일 안에 답을 드려요.</p>`}
            </div>` : ""}
        </div>`;
      }).join("")}
  ...
  home: () => {
    ...
    // ── 오늘이 어떤 날인가 — 가장 가까운 «큰» 일정 하나만 위에 건다
    const holi = (e) => S.holiOff && (e.closure_type || "").includes("공휴일");
    const nextBig = schedulesOf()
      .filter((e) => e.date >= TODAY && !holi(e) && !e.name.includes("토요휴업일")
        && classifyEvent(e.name, e.closure_type))
      .sort((a, b) => a.date.localeCompare(b.date))[0];

    // ── 큰 카드 ①  오늘 급식
    const meal = mealsOf().find((m) => m.date >= TODAY && m.type === "중식");
    const mealToday = meal && meal.date === TODAY;
    const mealTitle = !meal ? "급식 없음"
      : mealToday ? meal.dishes.slice(0, 2).map((x) => x.name).join(" · ")
      : `${+meal.date.slice(4, 6)}월 ${+meal.date.slice(6)}일부터`;
    const mealSub = mealToday ? meal.dishes.slice(2).map((x) => x.name).join(" · ") : "";

    /* ── 큰 카드 ②  하교 · 방과후
       하교 시각은 **지어내지 않는다** — 오늘 시간표에 든 마지막 교시의 끝 시각을 그대로 쓴다.
       시간표가 없으면(방학·미등록) 시각 없이 방과후만 말한다. */
    const todayWd = new Date(`${TODAY.slice(0,4)}-${TODAY.slice(4,6)}-${TODAY.slice(6)}`).getDay();
    const col = todayWd - 1;                       // 월=0 … 금=4. 주말이면 음수라 아래에서 걸린다
    const grid = ttOf()?.grid || [];
    const periods = (col >= 0 && col <= 4)
      ? grid.filter((row) => String(row?.[col] || "").trim()).length : 0;
    const outAt = periods ? STUB.periods[periods - 1]?.end : null;
    const todayAct = STUB.activities
      .filter((a) => String(a.days_of_week || "").split(",").includes(String(todayWd)));
    const afterTitle = todayAct[0]
      ? (outAt ? `${outAt} 하교 → ${todayAct[0].name}` : esc(todayAct[0].name))
      : (outAt ? `${outAt} 하교` : "오늘 수업 없음");
    // 학원 스텁에 «장소»는 없다(start_time·end_time 만 있다) — 없는 필드를 지어내지 않는다
    const afterSub = [todayAct[0]?.start_time, periods ? `오늘 ${periods}교시` : ""]
      .filter(Boolean).join(" · ");

    // ── 큰 카드 ③  내일 준비물
    const tm = ymdPlus(TODAY, 1);
    const packs = STUB.supplies.filter((x) => x.date === tm && !x.done);
    const packTitle = packs.length ? packs.map((x) => x.name || x.item).join(" · ") : "챙길 것 없음";

    /* 저녁(19시~) 챙길것 배너 — 하루 1회(닫은 날짜 기억). 캐러셀 스냅과 상단 슬롯이 같이 쓴다. */
    const showBanner = new Date().getHours() >= 19 && packs.length > 0
      && localStorage.getItem("eduthink.packBanner") !== TODAY;

    /* ── 미션 — 홈의 주인공(2026-08-02 개편). 진행률·도장판·확인 대기가 홈에서 바로 보인다.
       도장을 «찍는» 확정은 미션 화면이 한다(서버 판정·오조작 방지) — 여기선 어디를 눌러도 미션으로 간다. */
    const missions = S.missions;
    const missionDone = (missions || []).filter((x) => x.status === "done").length;
    const waiting = missionWaiting();

    /* ── 격자 — 홈 편집(S.menuOrder / S.menuHidden)의 뜻을 그대로 지킨다.
       탭·홈카드와 겹치던 항목은 HOME_MENU 목록 자체에서 빠졌다(위 정의 참고). */
    const unread = unreadFor("parent");
    const undone = STUB.documents.filter((d) => d.kind === "field" && !d.reported).length;
    /* 칸마다 아래에 붙는 한 줄. state 는 색을 켤 때만 준다 —
       "todo" = 빨강(해야 할 일·미확인) · "done" = 초록(끝남) · 없으면 회색. */
    const CELL = {
      notify:    { s: unread ? `안 읽음 ${unread}` : "새 소식 없음", state: unread ? "todo" : "", badge: unread },
      after:     { s: todayAct.length ? `오늘 ${esc(todayAct[0].name)}` : "" },
      docs:      undone ? { s: `안 낸 보고서 ${undone}`, state: "todo" } : { s: "낼 것 없음" },
      school:    { s: esc(c.school?.name || SCHOOL.info?.name || "") },
      community: { s: `${c.grade || ""}학년 방` },
    };
    const cells = homeMenuItems();
    const more = homeMoreItems();

    return `
    <div class="hd-top">
      <button class="hd-who" onclick="location.hash='#switch'">
    ...
    ${promoteCard(c)}
    <!-- nowCard() 를 뺐다(2026-08-03) — 「지금 뭐 하고 있나」는 아래 시간 축이 자리로 말한다.
         둘을 같이 두면 같은 말을 두 번 하고 첫 화면만 길어진다. 함수는 남아 있다. -->
    ${(() => {
      // 체험 D-day — 사흘 안쪽일 때만. 늘 띄우면 «광고»가 되어 눈이 지나친다(2026-07-31).
      const d = passDaysLeft(c);
      if (!passOpen(c) || d < 0 || d > 3) return "";
      return `<button class="hd-trial" onclick="location.hash='#pay'">
        체험 ${d === 0 ? "오늘까지" : `${d}일 남음`}<em>이용권 보기 ›</em></button>`;
    })()}

    ${(() => {
      /* 상단 한 슬롯(2026-08-02 지시) — 저녁 챙길것 배너 > D-day 행사.
         저녁 상태에서 둘을 겹치면 홈이 800px 를 넘었다(실측 830). 닫으면 D-day 가 돌아온다. */
      if (!showBanner) return "";
      return `<div class="hd-banner" role="status">
        <i aria-hidden="true" class="ti ti-backpack"></i>
        <span>내일 챙길 것 ${packs.length}개가 아직이에요</span>
        <button onclick="App.hdBannerClose()" aria-label="오늘 그만 보기"><i aria-hidden="true" class="ti ti-x"></i></button>
      </div>`;
    })()}
    ${!showBanner && nextBig ? `<button class="hd-dday" onclick="S.schoolTab='schedule';S.calMonth='${nextBig.date.slice(0,6)}';S.calPick='${nextBig.date}';location.hash='#school'">
      <i aria-hidden="true" class="ti ti-calendar"></i>
      <span class="hd-n">${esc(nextBig.name)}</span>
      <span class="hd-v">${ddayLabel(nextBig.date) || ""}</span>
    </button>` : ""}

    <!-- ── 오늘 한 줄 + 시간 축 (2026-08-03 개편) ────────────────────
         이전엔 카드 3장이 가로로 눕고 그 위에 도장판이 있었다 — 종류별 카드 더미라
         **어느 것부터 봐야 하는지**가 없었다. 아이 화면을 «한 장씩 덱»으로 바꾸면서
         부모 화면은 그 반대로 세운다: **하루를 위에서 내려다보는 한 판.**

         시간이 위에서 아래로 흐르고, 지난 일은 흐려지고, 지금 자리에 굵은 점이 선다.
         부모가 3시에 앱을 여는 이유는 「5교시가 뭐냐」가 아니라 「지금 어디 있냐」다 —
         그 답이 위치로 읽힌다. 아이콘·색이 아니라 **자리**가 뜻을 진다.
         ⚠ 시각은 지어내지 않는다. 시간표·급식·방과후에 실제로 있는 값만 쓴다. -->
    ${(() => {
      const now = new Date();
      const hhmm = String(now.getHours()).padStart(2, "0") + ":" + String(now.getMinutes()).padStart(2, "0");
      const lunch = mealToday ? (lunchOf().start || "12:30") : null;

      const rows = [];
      if (mealToday) rows.push({ t: lunch, ic: "ti-tools-kitchen-2", nm: "점심",
        sub: meal.dishes.slice(0, 3).map((x) => x.name).join(" · "),
        go: "S.schoolTab='meal';location.hash='#school'" });
      if (outAt) rows.push({ t: outAt, ic: "ti-school", nm: "하교",
        sub: periods ? `${periods}교시까지` : "",
        go: "S.schoolTab='timetable';location.hash='#school'" });
      todayAct.forEach((a) => rows.push({ t: a.start_time || "", ic: "ti-run", nm: esc(a.name),
        sub: a.end_time ? `${esc(a.start_time || "")}~${esc(a.end_time)}` : "",
        go: "location.hash='#afterschool'" }));

      // 시각이 있는 것만 정렬한다. 없는 것(준비물·미션)은 «저녁» 칸으로 뒤에 붙는다
      rows.sort((a, b) => String(a.t).localeCompare(String(b.t)));
      const passed = rows.filter((r) => r.t && r.t <= hhmm).length;
      const nowIdx = Math.min(passed, rows.length);   // 지금 «다음»에 올 줄

      rows.push({ t: "저녁", ic: "ti-backpack", nm: "내일 챙길 것",
        sub: packs.length ? packTitle : "챙길 것 없음", pend: packs.length > 0,
        go: "location.hash='#supplies'" });

      const line = rows.map((r, i) => {
        const done = r.t !== "저녁" && r.t && r.t <= hhmm;
        const isNow = i === nowIdx;
        return `
        <button class="hp-row ${done ? "past" : ""} ${isNow ? "now" : ""}" onclick="${r.go}">
          <span class="hp-t">${esc(String(r.t || ""))}</span>
          <span class="hp-mark"><i></i></span>
          <span class="hp-tx">
            <b>${r.nm}${r.pend ? ` <em class="hp-pend">${packs.length}</em>` : ""}</b>
            ${r.sub ? `<span>${esc(r.sub)}</span>` : ""}
          </span>
          <i aria-hidden="true" class="ti ti-chevron-right hp-go"></i>
        </button>`;
      }).join("");

      /* 한 문장 — 숫자 카드를 늘어놓는 대신 «지금 어떤가»를 말로 한 줄.
         읽는 데 1초면 되고, 그 아래 축을 볼지 말지는 부모가 정한다. */
      const nextRow = rows[nowIdx];
      const say = !rows.length ? "오늘은 학교 일정이 없어요"
        : nowIdx === 0 && rows[0].t !== "저녁" ? `곧 ${rows[0].nm}${rows[0].t ? ` · ${rows[0].t}` : ""}`
        : nextRow && nextRow.t !== "저녁" ? `다음은 ${nextRow.nm} · ${nextRow.t}`
        : "오늘 학교 일정은 끝났어요";

      return `
      <div class="hp-say">
        <b>${esc(c.nickname)}</b><span>${esc(say)}</span>
      </div>
      <div class="hp-line">${line}</div>`;
    })()}

    <!-- 미션 — 홈의 주인공은 그대로 미션이지만, 이제 «하루의 마지막 칸»으로 선다.
         도장 3칸은 남긴다(진행이 한눈에 보이는 자리라 글자보다 빠르다). -->
    <div class="hd-ms">
      <button class="hd-ms-hd" onclick="location.hash='#mission'">
        <b>오늘 미션</b>
        ${missions?.length ? `<span class="hd-ms-cnt">${missionDone}<i>/${missions.length}</i></span>` : ""}
        <i aria-hidden="true" class="ti ti-chevron-right hd-go"></i>
      </button>
      ${childLinkWarn()}
      ${missions === null ? `<div class="hd-ms-empty">불러오는 중…</div>`
        : !missions.length ? `<div class="hd-ms-empty">오늘 미션이 없어요</div>`
        : `<div class="hd-ms-board">
            ${missions.slice(0, 3).map((m) => {
              const st = m.status === "done" ? "on" : m.status === "waiting" ? "wait" : "off";
              return `<button class="hd-stamp ${st}" onclick="location.hash='#mission'"
                aria-label="${esc(m.title)} — ${st === "on" ? "완료" : st === "wait" ? "확인 대기" : "아직"}">
                <span class="mark">${st === "on" ? `<i aria-hidden="true" class="ti ti-check"></i>`
                  : st === "wait" ? `<i aria-hidden="true" class="ti ti-${m.verify === "review" ? "camera" : "clock"}"></i>`
          ...
          ${waiting ? (() => {
            const anyPhoto = (missions || []).some((m) => m.status === "waiting" && m.verify === "review");
            return `<button class="hd-ms-wait" onclick="location.hash='#mission'">
              <i aria-hidden="true" class="ti ti-${anyPhoto ? "camera" : "clock"}"></i>미션 ${waiting}개가 확인을 기다려요 — 도장 찍으러 가기</button>`;
          })() : ""}`}
    </div>

    <!-- 광고는 큰 카드 바로 아래 — 첫 화면 안에 들어와야 노출이 잡힌다(2026-07-31 실측). -->
    ${adSlot("home")}

    <!-- 더 보기 — 한 줄 소형 아이콘(2026-08-02 홈 다이어트). 상태 한 줄은 접었다 —
         빨간 점·뱃지가 그 역할을 대신한다. 상세는 눌러서 본다. -->
    <div class="hd-row5">
      ${cells.map((m) => {
        const k = CELL[m.key] || { s: "" };
        return `
        <button class="hd-mini" onclick="App.go('${jsq(m.key)}')" aria-label="${esc(m.label)}${k.s ? ` — ${k.s}` : ""}">
          <span class="hd-ic xs">
            <i aria-hidden="true" class="ti ${m.icon}"></i>
            ${k.badge ? `<span class="hd-badge">${k.badge}</span>`
              : k.state === "todo" ? `<span class="hd-st hd-todo"></span>`
              : ""}
          </span>
          <span class="hd-nm">${esc(m.label)}</span>
        </button>`;
      }).join("")}
    </div>

    <!-- 홈 편집에서 내린 것 — 없어진 게 아니라 여기로 온다(2026-07-25 규칙 유지) -->
    <div class="hd-more">
      <span class="hd-morelist">
        ${more.length ? more.map((m) => `<button onclick="App.go('${jsq(m.key)}')">${esc(m.label)}</button>`).join("<i>·</i>") : ""}
      </span>
      <button class="edit" onclick="location.hash='#homeedit'">편집</button>
    </div>
  ...
  homeedit: () => {
    const rows = S.menuOrder.map(menuOf).filter(Boolean);
    const shown = rows.filter((m) => !S.menuHidden.includes(m.key)).length;
    return `
    <a class="back" href="#home" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">홈 편집</h1>
    <p class="sub">끈 것은 없어지지 않고 <b>홈 맨 아래 한 줄</b>로 내려가요</p>

    <div class="card" style="padding:8px 10px">
      <div class="edhd"><b>홈 카드</b><span class="sub" style="margin:0">${shown}개</span></div>
      <div class="edlist" id="edlist">
        ${rows.map((m, i) => {
          const off = S.menuHidden.includes(m.key);
          return `
          <div class="edrow ${off ? "off" : ""}" data-i="${i}">
            <span class="edgrip" onpointerdown="App.menuDrag(event, ${i})"><i class='ti ti-grip-vertical'></i></span>
            <span class="edico" style="background:var(--c${m.ci});color:var(--f${m.ci})"><i class="ti ${m.icon}"></i></span>
            <span class="edname">${esc(m.label)}</span>
            <button class="swt ${off ? "" : "on"}" onclick="App.menuToggle('${m.key}')" aria-label="${esc(m.label)} ${off ? "켜기" : "끄기"}"><i></i></button>
          </div>`;
        }).join("")}
      </div>
    </div>
...
<div class="card" style="padding:8px 10px">
    ...
    <div class="row" style="margin-top:16px">
      <button class="btn-line" onclick="App.homeReset()">기본값으로</button>
...
`;
  ...
  calendar: () => { S.schoolTab = "schedule"; return Screens.school(); },
  ...
  school: () => {
    ...
    const body = {
      info: `
        ...
        <div class="card">
          <p class="sub" style="margin-top:0">${esc(sc.sido || "")} · ${esc(sc.kind || "")}<br>${esc(sc.address || "")}</p>
        ...
        <div class="card foldbox">
          <button class="foldhd" onclick="App.toggleGrades()">
            ...
            <span class="caret">${S.gradesOpen ? "▴" : "▾"}</span>
          </button>
          ${S.gradesOpen ? `
            <div class="gradelist">
    ...
    const elem = String(sc.kind || "").startsWith("초");
    const TABS = [["info", "학교정보"], ["meal", "급식"], ["schedule", "일정"], ["timetable", "시간표"]];
    if (elem) TABS.push(["community", "대화"]);
    return `
    ${childChip()}
    <h1>${esc(sc.name)} ${src}</h1>
    <div class="subtabs">
      ${TABS.map(([k, n]) => `<button class="${t === k ? "on" : ""}" onclick="App.schoolTab('${k}')">${n}</button>`).join("")}
    </div>
    ${body}`;
  },
  ...
  "child-add": () => {
    ...
    <div class="ca-top">
      <a class="ca-back" href="#${editing ? "mypage" : "switch"}">‹</a>
    ...
    <div class="card">
      ...
      <input id="school-q" placeholder="학교 이름을 입력하세요 (예: 금성초)" value="${esc(S.schoolQuery)}" oninput="App.schoolSearch(this.value)">
      ${(() => {
        ...
        if (!hits.length) return `<p class="sub">"${q}" 로 찾은 학교가 없어요. 이름을 다시 확인해 주세요.</p>`;
        return `
          <p class="sub">${hits.length}곳 찾았어요 — <b>주소를 보고</b> 우리 학교를 고르세요.
            ${S.schoolFrom === "stub" ? `<b class="stub">예시 목록이에요</b>` : ""}</p>
    ...
    <div class="charselect">
      ${STUB.children.map((c) => {
        ...
        <div class="char-card ${c.id === S.currentChildId ? "current" : ""}" onclick="App.pickChild(${c.id})">
          ...
          <div class="sc">${esc(c.school.name.replace("등학교", ""))}<br>${c.grade}학년 ${c.class_name}반</div>
  ...
  upload: () => {
    ...
    const stage = [
      ...
      `<div class="camera-box">
         ...
         <div style="font-size: var(--fs-sm)">누르면 <b>찍기·갤러리</b>를 그 자리에서 고를 수 있어요<br>종이는 테두리를 자동으로 잘라 올려요 (원본 색 유지)</div>
       </div>
       <div class="stickybar">
      ...
      `<!-- 앞·뒤 여러 장을 한 기록에 묶는다(2026-07-24). 통지표는 뒷면에도 내용이 있다. -->
       ...
       <p class="sub" style="margin-top:2px">사진을 누르면 크게 볼 수 있어요 · 종이는 자동으로 잘려요 · 현재 ${S.shots.length}장</p>
  ...
  timeline: () => {
    ...
    const hero = latest ? `
      <button class="dw-hero" onclick="App.openRecord(${latest.id})">
        ${latest.image ? `<img src="${latest.image}" ${latest._thumb ? `data-rec="${latest._thumb}"` : ""} alt="">`
          : `<span class="dw-noimg"><i aria-hidden="true" class="ti ti-file-text"></i>사진 없는 기록</span>`}
    ...
    <div class="dw-top">
      ...
      <span class="dw-acts"><button onclick="App.recordSearch()">${S.recSearch ? "닫기" : "검색"}</button><button onclick="location.hash='#home'">홈</button></span>
    </div>
    ${S.recSearch ? `
      <div class="dw-search">
        <i class='ti ti-search'></i>
    ...
    ${S.uploadStep === 1 && S.shots.length ? `
      <button class="dw-pending" onclick="location.hash='#upload'">
        <i class='ti ti-alert-triangle'></i> 찍어둔 사진 ${S.shots.length}장이 저장 전이에요<em>이어서 저장 ›</em>
      </button>` : ""}
    ${S.upFail ? `
      <button class="dw-pending" onclick="App.retryUpload()">
        <i class='ti ti-alert-triangle'></i> 기록 1건이 서버에 안 올라갔어요<em>다시 올리기 ›</em>
      </button>` : ""}
    ${hits ? `
      <div class="dw-hits">
...
`;
  ...
  supplies: () => {
    ...
    const line = (s) => `
      <label class="sp-row ${s.done ? "done" : ""}">
        <input type="checkbox" ${s.done ? "checked" : ""} onchange="App.supplyToggle(${s.id})">
        <i class="sp-box"></i>
        <span class="sp-t"><b>${esc(s.item)}</b>${s.memo ? `<span>${esc(s.memo)}</span>` : ""}</span>
        <button class="sp-del" onclick="event.preventDefault();App.supplyDelete(${s.id})"
                aria-label="${esc(s.item)} 지우기"><i class='ti ti-trash'></i></button>
      </label>`;
    ...
    <div class="sp-add">
      <input id="sup-i" placeholder="실내화 · 물감…" value="${esc(S.supDraft.item)}"
             oninput="S.supDraft.item=this.value" onkeydown="if(event.key==='Enter')App.supplyAdd()">
      <button onclick="App.supplyAdd()">추가</button>
    </div>
    <div class="sp-chips">
      <label class="sp-chip">${supDayLabel(S.supDraft.date)}
        <input type="date" id="sup-d" value="${ymdDash(S.supDraft.date)}" oninput="App.supDate(this.value)"></label>
      <!-- 반복은 **서버가 펼치지 않는다** — 앱이 실제 행을 4개 만든다(마이그레이션 주석 참고).
           체육복처럼 매주 같은 요일에 챙기는 것이 실제로 제일 많다. -->
      <button class="sp-chip" onclick="App.supplyRepeat(4)">매주 × 4주</button>
      <span class="sp-chip memo ${S.supMemoOpen ? "on" : ""}" onclick="App.supMemoToggle()">메모</span>
    </div>
    ${S.supMemoOpen ? `<input class="sp-memo" id="sup-m" placeholder="이름표 붙이기"
      value="${esc(S.supDraft.memo)}" oninput="S.supDraft.memo=this.value">` : ""}
  ...
  mypage: () => {
    ...
    <div class="mp-list">
      ${row("ti-photo", "테마", themeName, "App.themeOpen()")}
    ...
    <div class="mp-list">
      ...
      ${(() => {
        ...
        return `<div class="mp-row two"><span class="mp-l">혜택·소식 받기<em>끄면 그 채널로는 안 보내요</em></span>
          <span class="mk-chips">${chip("email", "메일")}${chip("sms", "문자")}${chip("call", "전화")}</span></div>`;
      })()}
      ${row("", "회원 탈퇴", "", "App.askDelete('회원 탈퇴 — 로그인이 막히고, 데이터는 3개월 보관 후 완전히 파기됩니다.')", "danger")}
    </div>

    ${S.nameDraft === null ? "" : `
    <div class="modal sheetwrap" onclick="App.nameCancel()">
      <div class="sheet" onclick="event.stopPropagation()">
        <div class="sheet-grip"></div>
        <div class="sheet-hd"><b>이름</b><span>앱 안에서만 써요</span></div>
        <input class="sheet-name" id="name-edit" maxlength="20" placeholder="예: 김하늘" value="${esc(S.nameDraft)}"
               oninput="S.nameDraft=this.value" onkeydown="if(event.key==='Enter')App.nameSave()">
        <div class="sheet-act nodel">
          <button class="act-cancel" onclick="App.nameCancel()">취소</button>
          <button class="act-save" onclick="App.nameSave()">저장</button>
        </div>
      </div>
    </div>`}

    ${!S.themeSheet ? "" : `
    <div class="modal sheetwrap" onclick="App.themeClose()">
      <div class="sheet" onclick="event.stopPropagation()">
        <div class="sheet-grip"></div>
        <div class="sheet-hd"><b>테마</b><span>밝게 · 어둡게</span></div>
        <div class="themegrid">
          <!-- ⚠ 이 값은 style.css 의 --bg/--accent 와 **같아야 한다.** 미리보기가 실제와 다르면 고른 뜻이 사라진다.
               2026-08-02: 6종 → 2종. 옛 키(warmgray·lavender 등)는 style.css 가 두 벌에 매달아 둬서
               저장돼 있던 사람도 화면이 안 튄다. 여기 목록에서만 빠진 것이다. -->
          ${[["light", "밝게", "#FFFFFF", "#2F6BEA"],
             ["dark", "어둡게", "#16181D", "#5C93FF"]].map(([t, nm, bg, ac]) => `
            <button class="themeswatch ${(DARK_THEMES.includes(S.theme) ? "dark" : "light") === t ? "on" : ""}" onclick="App.setTheme('${jsq(t)}')">
              <span class="tprev" style="background:${bg}"><i style="background:${ac}"></i></span>
              <span class="tn">${nm}</span>
            </button>`).join("")}
        </div>
        <div class="sheet-act nodel" style="grid-template-columns:1fr">
          <button class="act-save" onclick="App.themeClose()">완료</button>
        </div>
      </div>
    </div>`}`;
  },
  ...
  report: () => {
    ...
    if (!r || S.reportBusy) return `
      <a class="back" href="#mission" style="font-size:22px;color:var(--muted)">‹</a>
    ...
    if (!total) return `
      ...
      <div class="rp-empty">
        ...
        <b>아직 모으는 중이에요</b>
        <span>아이가 매일 퀘스트를 하면 쌓입니다.<br>열흘쯤 모이면 리포트가 나와요.</span>
      </div>`;
    ...
    <div class="rp-sec">
      ...
      ${r.meal.dishes?.length ? `<p class="rp-lead">두 번 이상 나온 <b>메뉴</b></p>
        ...
        : `<p class="rp-note">아직 두 번 이상 나온 메뉴가 없어요. 급식은 겹치는 게 적어서 한 달은 모여야 보입니다.</p>`}
    </div>` : ""}
    ...
    <div class="rp-sec">
      ...
      <p class="rp-note">아이가 직접 고른 이름입니다. 학교에 확인한 것이 아니에요.</p>
    </div>` : ""}
    ...
    <div class="rp-sec">
      ...
      <p class="rp-note">성적이 아니라 <b>재미</b>입니다. 잘하는 것과 다를 수 있어요.</p>
    </div>` : ""}`;
  },
  ...
  pay: () => {
    ...
    return `
    <a class="back" href="#mypage" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">이용권</h1>
    <p class="sub">자녀 1명당 하나씩 — 지금은 <b>${esc(c.nickname)}</b> 이용권을 골라요</p>
    ${seat > 1 ? `<p class="seatnote"><i class='ti ti-discount'></i> <b>${seat}번째 자녀</b>라 더 저렴해요 — 아이가 늘수록 값이 내려갑니다</p>` : ""}
    ${list.map((p) => `
    ...
    <div style="margin-top:16px"><button class="btn-primary" ${S.payBusy ? "disabled" : ""} onclick="App.buyPlan()">
      ${S.payBusy ? "결제 확인 중…" : (BILLING.on ? "구글 플레이로 결제" : "결제 준비 중")}</button></div>
    <p class="mp-hint">자동으로 갱신되지 않아요 · 구글 플레이 결제 · 카드번호 등 결제수단 정보는 저희 서버에 저장되지 않습니다</p>`;
  },
  ...
  docs: () => {
    ...
    return `
    <a class="back" href="#home" style="font-size:22px;color:var(--muted)">‹</a>
    <h1 style="margin-top:8px">체험학습 · 출결서식</h1>
    ...
    <p class="mp-hint">${esc(c.school.name)} 2026학년도 서식 기준 · 학교마다 다를 수 있어 참고용입니다</p>`;
  },
  ...
  "doc-form": () => {
    ...
    const auto = `
      <div class="card autofill">
        ...
        <div class="afrow"><span>학교</span><b>${esc(c.school.name)}</b></div>
    ...
    const fieldApply = () => {
      ...
      <div class="card">
        ...
        ${overQuota ? `<p class="warnline"><i class='ti ti-alert-triangle'></i>️ 올해 남은 일수(${left}일)를 넘습니다 — 초과분은 <b>미인정 결석</b>이에요.</p>` : ""}
        ${overRun ? `<p class="warnline"><i class='ti ti-alert-triangle'></i>️ 연속 ${R.fieldMaxRun}일을 넘을 수 없어요.</p>` : ""}
        ...
        ${d.accompany === "아니오"
          ? `<p class="warnline"><i class='ti ti-alert-triangle'></i>️ 보호자(또는 친족 성인) 동행이 없으면 <b>허가되지 않습니다</b>.</p>`
    ...
    const fieldReport = () => `
      ...
      <div class="card">
        ...
        <textarea rows="6" placeholder="교육활동 내용이 잘 드러나도록 자세히 적어주세요.&#10;예: 성산일출봉에서 화산 지형을 관찰하고…"
      ...
      <div class="card">
        ...
        ${t === "sick" ? `
          <p class="${needCert ? "warnline" : "sub"}">${needCert
            ? `<i class='ti ti-alert-triangle'></i>️ ${days}일이면 <b>의사 진단서 또는 의견서</b>가 필요해요 — 약봉투·처방전은 인정되지 않습니다.`
        ...
        ${t === "none" ? `<p class="warnline"><i class='ti ti-alert-triangle'></i>️ 미인정 결석입니다. 연속 3일 이상이면 학교가 보호자·학생 면담, 가정방문 등으로 소재를 확인합니다.</p>` : ""}
    ...
    const counsel = () => {
      ...
      <div class="card">
        ...
        <textarea rows="5" placeholder="상담을 신청하시는 이유와 자녀에 관한 의견을 적어주세요."
    ...
    return `
    <a class="back" href="#docs" style="font-size:22px;color:var(--muted)">‹</a>
  ...
  "doc-preview": () => `
    <div class="pvbar">
      <a class="back" href="#doc-form" style="font-size:22px;color:var(--muted)">‹</a>
      <b>${docTitle()}</b>
  ...
  notify: () => {
    ...
    <div class="nt-send">
      ...
      <button onclick="App.sendNotice()">보내기</button>
    </div>
    ...
    <div class="mp-list">
      <button class="mp-row" onclick="App.pushRegister()">
        ...
        ${S.pushToken ? "" : `<span class="mp-go">›</span>`}
      </button>
  ...
  childtt: () => {
    ...
    return `
    <a class="back" href="#childview" style="font-size:22px;color:var(--muted)">‹</a>
  ...
  childmealrate: () => {
    ...
    <div class="kd2-face">${FACE[d.stars] || "🍽️"}</div>
    ...
    <button class="kd2-cta" ${d.stars && !S.mealBusy ? "" : "disabled"} onclick="App.mealRateSubmit()">
      ${S.mealBusy ? "보내는 중…" : `다 했어요 · ${kw("unit")} 1개`}</button>`;
  },
  ...
  childsubject: () => {
    ...
    <button class="kd2-cta" ${S.subjectBest && !S.subjectBusy ? "" : "disabled"}
      onclick="App.subjectSubmit()">${S.subjectBusy ? "보내는 중…" : `다 했어요 · ${kw("unit")} 1개`}</button>`;
  },
  ...
  childfriend: () => {
    ...
    <button class="kd2-cta" ${(d.picked.length || d.alone) && !S.friendBusy ? "" : "disabled"}
      onclick="App.friendSubmit()">${S.friendBusy ? "보내는 중…" : `다 했어요 · ${kw("unit")} 1개`}</button>`;
  },
  ...
  childlocked: () => `
    <div class="kd2-end">
      ...
      <p class="kd2-sub">고장난 게 아니에요.<br><b>엄마아빠에게 말해주면</b> 다시 열려요.</p>
    </div>`,
  ...
  childexpired: () => `
    <div class="kd2-end">
      ...
      <p class="kd2-sub">괜찮아요, 고장난 게 아니에요.<br>부모님 폰에서 <b>새 코드</b>를 받아서 다시 이어요.</p>
  ...
  childview: () => {
    ...
    <div class="kd3-deck">
      ...
      <div class="kd3-stack">
        ...
        <article class="kd3-card">
          ...
          <div class="kd3-foot">
            ${card.skip ? `<button class="kd3-x" onclick="${card.skip}">못 했어요</button>` : `<span></span>`}
    ...
    ${(() => {
      ...
      <div class="kd2-supadd">
        ...
        <button onclick="App.childSupplyAdd()">넣기</button>
      </div>`;
    })()}
...
function childLinkWarn() {
  ...
  return `
  <button class="ms-linkwarn" onclick="location.hash='#mypage'">
    <i aria-hidden="true" class="ti ti-alert-triangle"></i>
    <span>자녀폰 미연결 — 사진 미션은 확인받을 수 없어요</span>
    <em>연결하기 ›</em>
  </button>`;
}
...
function childChip() {
  ...
  return `<div class="childchip" onclick="location.hash='#switch'">
    ${childFace(c, "avatar")} ${esc(c.nickname)} <span style="color:var(--muted)">▾</span>
  </div>`;
}
// ── 방과후 주간 타임그리드 ──────────────────────────────────
...
function planBar() {
  ...
  return `
  <div class="modal sheetwrap" onclick="App.planCancel()">
   <div class="sheet" onclick="event.stopPropagation()">
    <div class="sheet-grip"></div>
    <!-- ⚠ 예전엔 제목에 DAY_KO[d.day - 1] 을 박아 썼는데, 「줄 추가」로 들어오면 d.day 가 없어
         **「undefined요일 일정」** 이 떴다(2026-07-28 사장님 지적). 고른 요일을 그대로 읽어준다. -->
    <div class="sheet-hd"><b>${d.id ? "일정 고치기" : "새 일정"}</b>
      <span>${(d.days || []).length ? d.days.map((n) => DAY_KO[n - 1]).join(" ") : "요일을 골라주세요"}</span></div>

    <input class="sheet-name" id="pb-name" placeholder="학원·활동 이름"
           value="${esc(d.name)}" oninput="S.planDraft.name=this.value">
    <div class="sheet-two">
      <label><span>시작</span><input id="pb-s" type="time" value="${d.start}" oninput="S.planDraft.start=this.value"></label>
      <label><span>끝</span><input id="pb-e" type="time" value="${d.end}" oninput="S.planDraft.end=this.value"></label>
    </div>

    <!-- 요일 — 학원은 「월·수」처럼 여러 날에 걸친다. 예전엔 한 요일만 담겨서
         월요일과 수요일 피아노가 **다른 학원 두 개**가 됐다(2026-07-28 통합). -->
    <div class="sheet-lbl">요일</div>
    <div class="sheet-days">
      ${DAY_KO.map((n, i) => `
        <button class="${(d.days || []).includes(i + 1) ? "on" : ""}" onclick="App.planDay(${i + 1})">${n}</button>`).join("")}
    </div>

    <div class="sheet-lbl">색</div>
    <div class="swatches">
      ${PLAN_COLORS.map((c, i) => `
    ...
    <div class="sheet-act ${d.id ? "" : "nodel"}">
      ${d.id ? `<button class="act-del" onclick="App.planDelete()">삭제</button>` : ""}
      <button class="act-cancel" onclick="App.planCancel()">취소</button>
      <button class="act-save" onclick="App.planSave()">${d.id ? "저장" : "추가"}</button>
    </div>
   </div>
  </div>
`;
...
function nowCard() {
  ...
  if (!n) return "";
  return `
  <button class="nowbar ${n.k}" onclick="S.schoolTab='timetable';location.hash='#school'">
...
function docPaper() {
  ...
  const 귀하 = `<p class="to">${school}장 귀하</p>`;
...
function communityTab() {
  ...
  <div class="cm-wrap">
    <div class="cm-hd">
      ...
      <button onclick="App.commLeaveAsk()">나가기</button>
    </div>
...
function diaryTab() {
  ...
  const cell = (key, i) => {
    ...
    return `
    <button class="gc-cell ${key === pick ? "on" : ""} ${key === TODAY ? "today" : ""}" onclick="App.calPick('${key}')">
  ...
  const agenda = days.map((r) => `
    <div class="gc-day ${r.key === pick ? "on" : ""}" onclick="App.calPick('${r.key}')">
  ...
  <div class="gc-wk">${["일","월","화","수","목","금","토"].map((d, i) => `<span class="${i === 0 ? "sun" : ""}">${d}</span>`).join("")}</div>
  ...
  <div class="gc-agenda">
    ${agenda || `<div class="gc-none">${m}월에는 일정이 없어요</div>`}
    <button class="gc-next" onclick="App.calMove(1)">${m === 12 ? 1 : m + 1}월 보기 ›</button>
  </div>

  <button class="gc-fab" onclick="App.evStart('${pick}')" aria-label="일정 넣기">＋</button>
  ${(() => {
    const e = S.evDraft;
    if (!e) return "";
    return `
    <div class="modal" onclick="App.evCancel()">
     <div class="modal-card" onclick="event.stopPropagation()">
      <div class="planbar-row"><b>${+e.date.slice(4, 6)}/${+e.date.slice(6)}${e.id ? " 일정 수정" : " 일정 넣기"}</b>
        <input id="ev-t" placeholder="일정 이름 (예: 치과)" value="${esc(e.title)}" oninput="S.evDraft.title=this.value"></div>
      <div class="planbar-row"><input id="ev-memo" placeholder="메모 (선택)" value="${esc(e.memo)}" oninput="S.evDraft.memo=this.value"></div>
      <div class="planbar-row">
        <input type="time" value="${e.start}" oninput="S.evDraft.start=this.value"><span>~</span>
        <input type="time" value="${e.end}" oninput="S.evDraft.end=this.value">
      </div>
      <!-- 알림(2026-07-31 «알람추가 왜없어») — 고른 만큼 시작시각에서 당겨 알림을 예약한다.
           보내는 쪽은 서버 크론이라 앱을 꺼둬도 온다. 「전날 저녁」은 준비물 챙기라고 미리 알리는 자리. -->
      <div class="planbar-row alarmrow">
        <span class="alarm-l"><i class='ti ti-bell'></i> 알림</span>
        <select onchange="S.evDraft.remind=this.value;App.render()">
          ${[["", "없음"], ["0", "시작할 때"], ["10", "10분 전"], ["30", "30분 전"], ["60", "1시간 전"], ["prev", "전날 저녁 8시"]]
            .map(([v, nm]) => `<option value="${v}" ${String(e.remind ?? "") === v ? "selected" : ""}>${nm}</option>`).join("")}
        </select>
      </div>
      <div class="planbar-row actrow">
        ${e.id ? `<button class="btn-danger" onclick="App.evDelete()">삭제</button>` : ""}
        <button class="btn-ghost" onclick="App.evCancel()">취소</button>
        <button class="btn-primary" onclick="App.evSave()">${e.id ? "저장" : "추가"}</button>
      </div>
     </div>
    </div>`;
  })()}`;
...
function timetableTab() {
  ...
  <div class="${schoolOpen ? "" : "hide"}">
    ...
    ${changed ? `
      <div class="tt-reset"><span>${changed}칸 고침</span><button onclick="App.ttReset()">되돌리기</button></div>` : ""}
  </div>

  <!-- 시안 10c (2026-07-28) — **아래에서 올라오는 시트.** 어느 칸을 고치는지는
       제목(「수요일 3교시」)과 격자에 남은 강조 링이 말한다. 과목은 3열 격자 48px,
       고른 것은 ✓ + 이중 링(7f). 버튼은 비우기 1 : 취소 1 : 저장 2. -->
  ${e ? (() => {
    const nowV = at(e.r, e.c);
    const t = STUB.periods[e.r] || {};
    const list = SUBJECTS.filter((s) => s !== "빈칸");
    return `
  <div class="modal sheetwrap" onclick="App.ttClose()">
    <div class="sheet" onclick="event.stopPropagation()">
      <div class="sheet-grip"></div>
      <div class="sheet-hd">
        <b>${DAY_KO[e.c]}요일 ${e.r + 1}교시</b>
        ${t.start ? `<span>${t.start} – ${t.end}</span>` : ""}
      </div>
      <div class="tt-pick">
        ${list.map((s) => `<button class="${nowV === s ? "on" : ""}" onclick="App.ttPick('${jsq(s)}')">${s}</button>`).join("")}
        <button class="own ${S.ttOwnOpen ? "on" : ""}" onclick="App.ttOwnToggle()">직접 쓰기</button>
      </div>
      ${S.ttOwnOpen ? `<input class="sheet-name" id="tt-own" placeholder="예: 방과후 독서" value="${esc(S.ttOwn)}"
             oninput="S.ttOwn=this.value" onkeydown="if(event.key==='Enter')App.ttPickOwn()">` : ""}
      <div class="sheet-act">
        <button class="act-del" onclick="App.ttPick('빈칸')">비우기</button>
        <button class="act-cancel" onclick="App.ttClose()">취소</button>
        <button class="act-save" onclick="App.ttPickOwn()">저장</button>
      </div>
    </div>
  </div>`;
  })() : ""}
...
function dayPanel(key) {
  ...
  <div class="dp">
    ...
    <button class="dp-add" onclick="App.evStart('${key}')"><i class='ti ti-plus'></i> 이 날 일정 넣기</button>
  </div>`;
}
...
const AUTH_FREE = ["login", "register", "findpw", "childlink", "terms", "policy", "help", "childexpired"];
...
const passOpen = (c) => !!c?.pass?.active;
...
const App = {
  render() {
    ...
    if (!name || !Screens[name]) name = S.loggedIn ? "home" : "login";
    if (!S.loggedIn && !AUTH_FREE.includes(name)) name = "login";
    ...
    const showTabs = S.loggedIn && !AUTH_FREE.includes(name) && !isChildToken()
  ...
  hdBannerClose() {
    // «오늘 그만» — 내일이 되면 다시 뜬다. 슬롯을 D-day 행사에 돌려줘야 하므로 다시 그린다.
    localStorage.setItem("eduthink.packBanner", TODAY);
    this.render();
  },
  ...
  findpwSend() {
    ...
    if (!/^[a-zA-Z0-9]{4,}$/.test(f.userid.trim())) f.err.userid = "가입한 아이디를 적어주세요";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email.trim())) f.err.email = "이메일 형식이 맞지 않아요";
    if (Object.keys(f.err).length) { this.render(); return; }
    f.step = 2; f.code = ""; this.render();
    toast("확인 코드를 보냈어요 (서버가 붙으면 실제 메일이 갑니다)");
  },
  ...
  findpwKakao() {
    ...
    S.findpw.step = 3;
    this.render();
    toast("카카오 본인 확인 완료 — 새 비밀번호를 정해주세요");
  },
  findpwVerify() {
    const f = S.findpw; f.err = {};
  ...
  schoolTab(t) { S.schoolTab = t; this.render(); },
  ...
  schoolSearch(v) {
    ...
    this._schoolTimer = setTimeout(async () => {
      try {
        ...
        // 서버가 없거나 못 붙었다 — 목업이 멈추지 않게 스텁으로 찾는다
        S.schoolHits = STUB.schoolSearch.filter((s) => s.name.includes(q) || s.name.replace("초등학교", "초").includes(q));
        S.schoolFrom = "stub";
      }
      S.schoolBusy = false;
  ...
  schoolPick(slug) {
    ...
    S.childDraft.school = { ...s, detail: schoolInfo().detail || STUB.school.detail };
  ...
  childEditStart(id) {
    ...
    S.childDraft = {
      ...
      agree: true, school: { ...c.school, detail: schoolInfo().detail || STUB.school.detail },
  ...
  nameEdit() { S.nameDraft = STUB.me.account.display_name || ""; this.render(); document.getElementById("name-edit")?.focus(); },
  nameCancel() { S.nameDraft = null; this.render(); },
  async nameSave() {
  ...
  async missionDone(id) {
    ...
    if (d.status === "waiting") {
      toast(isChildToken() || S.childView
        ? "확인중이에요! 늦어도 아침 8시에는 " + kw("unit") + "이 나와요"
        : "확인 기다림으로 넘어갔어요");
      return;
    }
  ...
  async missionPickOpen() {
    ...
    } catch (_) { S.missionCatalog = []; }
    this.render();
  },
  missionPickClose() { S.missionCatalog = null; S.missionPick = []; S.missionNew = null; this.render(); },
  ...
  missionToggle(code) {
    ...
    else if (S.missionPick.length >= 3) return toast("세 개까지만 고를 수 있어요");
  ...
  async missionSave() {
    ...
    try {
      ...
      S.missionCatalog = null; S.missionPick = [];
      await this.loadMissions(true);
      toast("오늘 미션을 바꿨어요");
  ...
  calPick(d) { S.calPick = d; S.evDraft = null; this.render(); },
  ...
  evEdit(id) {
    ...
    if (!e) return;
    S.evDraft = { id: e.id, date: e.date, title: e.title, memo: e.memo || "",
      start: e.start || "16:00", end: e.end || "17:00", remind: e.remind ?? "" };
    this.render(); document.getElementById("ev-t")?.focus();
  },
  evDelete() {
    const d = S.evDraft;
  ...
  ttOpen(r, c) {
    ...
    S.ttEdit = { r, c };
    // 지금 값이 목록에 없는 과목이면 「직접 쓰기」를 펴 둔 채 시작한다 — 사용자가 적은 것이 안 보이면 안 된다
    S.ttOwn = SUBJECTS.includes(v) ? "" : v;
    S.ttOwnOpen = !!S.ttOwn;
    this.render();
    if (S.ttOwnOpen) document.getElementById("tt-own")?.focus();
  },
  ttClose() { S.ttEdit = null; S.ttOwn = ""; S.ttOwnOpen = false; this.render(); },
  ttOwnToggle() {
  ...
  themeOpen() { S.themeSheet = true; this.render(); },
  themeClose() { S.themeSheet = false; this.render(); },
  ...
  planCancel() { S.planDraft = null; this.render(); },
  ...
  planEdit(id) {
    ...
    this.render(); document.getElementById("pb-name")?.focus();
  },
  ...
  planSave() {
    ...
    if (TOKENS.access && c0?.server_id) {
      ...
      } }).then((r) => { if (!r?.ok) toast(r?.error?.message || "학원을 서버에 저장 못 했어요"); });
    }
    toast("추가했어요");
  },
...
function loadKidPrefs() {
  try { const v = JSON.parse(localStorage.getItem(KID_PREF_KEY) || "null");
    if (v && KID_THEMES[v.theme]) { S.kidTheme = v.theme; S.kidAccent = v.accent; } } catch (e) {}
}
...
function applyKidTheme(on) {
  ...
  if (on && S.kidTheme) { el.dataset.kidTheme = S.kidTheme; el.dataset.kidAccent = S.kidTheme; }
...
function setChildMe(child) {
  ...
  STUB.children = [{
    ...
    school: child.school || null, counts: {}, pass: null, photo: null,
  }];
  S.currentChildId = 1;
}
...
document.addEventListener("visibilitychange", () => {
  ...
  const busy = S.planDraft || S.lunchEdit || S.promote || S.ttEdit || S.promoteSheet
    || S.uploadStep || S.viewRecord || S.nameDraft !== null || S.themeSheet
