...
function adminLayout({ title, active, body, badges = {} }) {
  const TABS = [
    ...
    ["data", "데이터", "/admin/data"],
    // 감사 기록 (2026-08-10) — 손으로 이용권을 주고 회수하는 이상 그 기록이 보이는 곳이 있어야 한다.
    ["audit", "기록", "/admin/audit"],
  ];
  const badgeOf = (key) => {
  ...
  const body = `
    <div class="hd"><h2>기록</h2>
      <form method="get" style="display:flex;gap:8px">
        <input class="btn" style="cursor:text;min-width:200px" name="q" value="${escapeHtml(q)}" placeholder="사유·대상·관리자">
    ...
    <div class="card">
      ${!list.length ? `<div class="empty" style="padding:28px">기록이 없습니다</div>` : list.map((r) => `
        <div class="trow" style="padding:12px 14px;align-items:flex-start">
          <span style="width:132px;font-size:13px;color:#6b7280">${escapeHtml(r.atF)}</span>
          <span style="width:96px;font-size:13.5px;font-weight:700">${escapeHtml(AUDIT_KO[r.action] || r.action)}</span>
          <span style="flex:1;min-width:0">
            <div style="font-size:14px">${escapeHtml(r.detail || "")}</div>
            <div style="font-size:13px;color:${r.reason ? "#6b7280" : "#b91c1c"};margin-top:2px">
              ${r.reason ? "사유 " + escapeHtml(r.reason) : "사유 없음"}</div>
          </span>
          <span style="width:150px;font-size:12.5px;color:#6b7280;text-align:right">
            ${escapeHtml(r.targetKind || "")} ${escapeHtml(r.targetShort || "")}<br>${escapeHtml(r.actor)} · ${escapeHtml(r.ip || "")}
          </span>
        </div>`).join("")}
    </div>`;
