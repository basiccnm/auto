...
/* ── 2단계: «묶음 안에서 죽은 선택자» 빼기 (2026-08-09) ─────────────
   `.card, .tk, .msc { ... }` 처럼 여럿을 묶어 쓴 규칙은 1단계로 못 자른다 —
   한 이름만 지우고 싶어도 줄 전체가 셋을 공유하기 때문이다.
   하지만 그중 **어떤 이름의 속성이 뒤에서 전부 다시 정해졌다면** 그 이름은
   이 줄에서 아무 일도 안 한다 → **이름만 빼면 된다.** 나머지 선택자는 그대로다.
   ⚠ 이름을 빼도 «남는 선택자가 하나 이상» 이어야 한다. 다 빠지면 규칙째 사라져야 하는데,
     그건 1단계의 «빈 껍데기 정리»가 이미 하는 일이라 여기선 최소 1개를 남긴다.
   실행:  node scripts/css_untangle.mjs --groups
   ────────────────────────────────────────────────────────────── */
